...
/* ============ LIBRARIAN ============ */
function LibrarianDashboard({ store, go }) {
...
function LibrarianCatalogue() {
  const C = window.CAMPUS_DATA;
  const [q, setQ] = React.useState("");
  const rows = C.BOOKS.filter((b) => b.title.toLowerCase().includes(q.toLowerCase()) || b.author.toLowerCase().includes(q.toLowerCase()));
  return (
    <div className="u-content">
      <PageHead title="Catalogue" sub={C.LIB_STATS.titles.toLocaleString() + " titles · " + C.LIB_STATS.copies.toLocaleString() + " copies"}>
        <Btn variant="accent" icon="plus">Add title</Btn>
      </PageHead>
