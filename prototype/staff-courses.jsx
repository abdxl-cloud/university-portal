...
function CourseManage({ code, store, actions, onBack }) {
  const { STAFF_COURSES, ENROLLED } = window.STAFF_DATA;
  const course = STAFF_COURSES.find((c) => c.code === code);
                    ...
                    <div key={s.matric} className="u-row u-wrap" style={{ gap: 10, padding: "12px 0", borderTop: "1px solid var(--border)", justifyContent: "space-between" }}>
                      <div className="u-row" style={{ gap: 10, minWidth: 0 }}>
                        ...
                        <div style={{ minWidth: 0 }}>
                          ...
                          <div className="u-meta">{s.status === "missing" ? "No submission" : s.fileName + " · " + s.at}</div>
                        </div>
                      </div>
...
Object.assign(window, { StaffCourses, CourseManage, StaffResults });
