...
function siwesStatusTag(status) {
  ...
  return <Tag variant={s.tone} dot>{s.label}</Tag>;
}

function Siwes({ store, actions }) {
  const S = window.SIWES_DATA;
  const w = store.siwes || { status: "none", placement: null, logbook: [], checkins: [] };
  const [tab, setTab] = React.useState("placement");
  ...
  return (
    <div className="u-content">
      ...
      {tab === "placement" && <SiwesPlacement store={store} actions={actions} w={w} />}
      {tab === "logbook" && <SiwesLogbook store={store} actions={actions} w={w} />}
      {tab === "checkin" && <SiwesCheckin store={store} actions={actions} w={w} />}
    </div>
  );
}

/* ---------- PLACEMENT: propose company \u2192 dept approval workflow ---------- */
function SiwesPlacement({ store, actions, w }) {
  const S = window.SIWES_DATA;
  const [form, setForm] = React.useState({ company: "", address: "", industry: "", supervisorName: "", supervisorPhone: "", supervisorEmail: "", startDate: "", endDate: "" });
  const set = (k, v) => setForm((f) => ({ ...f, [k]: v }));
  const fillSample = (c) => setForm((f) => ({ ...f, company: c.name, address: c.address, industry: c.industry }));
...
function SiwesLogbook({ store, actions, w }) {
  const [date, setDate] = React.useState("");
  const [activity, setActivity] = React.useState("");
  const [hours, setHours] = React.useState(6);
  const active = ["ongoing", "completed"].includes(w.status);

  const submit = () => { if (!date || !activity.trim()) return; actions.siwesAddLog({ date, activity: activity.trim(), hours: Number(hours) || 0 }); setDate(""); setActivity(""); setHours(6); };
  ...
  const STATUS_TONE = { submitted: "accent", signed: "success", returned: "danger" };
  return (
            ...
            {w.logbook.map((l) => (
              <div key={l.id} className="u-li" style={{ padding: "13px 22px", flexDirection: "column", alignItems: "stretch", gap: 6 }}>
                <div className="u-row" style={{ justifyContent: "space-between" }}>
                  ...
                  <div className="u-row" style={{ gap: 8 }}><span className="u-meta">{l.hours}h</span><Tag variant={STATUS_TONE[l.status]} dot>{l.status}</Tag></div>
                </div>
...
function SiwesCheckin({ store, actions, w }) {
  const S = window.SIWES_DATA;
  const [checking, setChecking] = React.useState(false);
  const active = ["ongoing", "completed"].includes(w.status);

  const doCheckIn = (matched) => {
    setChecking(true);
  ...
  return (
    <div className="u-grid" style={{ gridTemplateColumns: "1fr 1.2fr", gap: 16, alignItems: "start" }}>
      <Card className="u-pad">
        ...
        <div className="u-ph" style={{ height: 140, marginBottom: 16 }}>map preview · {S.PLACEMENT_COORD.lat.toFixed(4)}, {S.PLACEMENT_COORD.lng.toFixed(4)}</div>
