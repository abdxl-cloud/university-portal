...
function allLibBooks(store) {
  ...
  const added = (store.campus && store.campus.libBooks) || [];
  return [...added, ...C.BOOKS];
}
function allCollections(store) {
  const C = window.CAMPUS_DATA;
  const extra = (store.campus && store.campus.libNewColl) || [];
  return [...C.LIB_COLLECTIONS, ...extra];
}
function collectionBookIds(store, coll) {
  const added = (store.campus && store.campus.libColl && store.campus.libColl[coll.id]) || [];
  return [...(coll.seed || []), ...added];
}

/* ---------------- ADD TO COLLECTION (popup) ---------------- */
function AddToCollectionModal({ store, actions, onClose }) {
  const C = window.CAMPUS_DATA;
  const [mode, setMode] = React.useState("scan"); // scan | manual
  const blank = { isbn: "", title: "", author: "", publisher: "", year: "", cat: "Computer Science", call: "", copies: 2, shelf: "" };
  const [form, setForm] = React.useState(blank);
  const [scanState, setScanState] = React.useState("idle"); // idle | scanning | found | notfound
  const [scanInput, setScanInput] = React.useState("");
  const [colls, setColls] = React.useState(["new"]);
  const [saved, setSaved] = React.useState(null);

  const set = (k, v) => setForm((f) => ({ ...f, [k]: v }));
  const toggleColl = (id) => setColls((c) => c.includes(id) ? c.filter((x) => x !== id) : [...c, id]);
  ...
  const runScan = (code) => {
    ...
    setScanInput(isbn);
    setTimeout(() => {
      const hit = C.ISBN_DB[isbn];
      if (hit) { setForm({ ...blank, isbn, ...hit, copies: 2, shelf: "" }); setScanState("found"); setColls(["new"]); }
      ...
      {saved ? (
        <div className="u-pad">
          ...
          <div className="u-row u-wrap" style={{ gap: 6, marginTop: 12 }}>
            {saved.collections.map((cid) => { const c = collections.find((x) => x.id === cid); return c ? <Tag key={cid} variant="accent"><Icon name={c.icon} size={12} /> {c.name}</Tag> : null; })}
          </div>
              ...
              <div className="u-grid u-grid--2" style={{ gap: 14 }}>
                ...
                <Field label="Category">
                  <select className="fb-input" value={form.cat} onChange={(e) => set("cat", e.target.value)}>
                    {C.LIB_CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
                  </select>
                </Field>
                ...
                {collections.map((c) => {
                  ...
                  return (
                    <button key={c.id} className="u-coll-pick" data-on={on} onClick={() => toggleColl(c.id)}>
                      <span className="u-icon" style={{ width: 30, height: 30 }}><Icon name={c.icon} size={15} /></span>
...
function LibraryShelf({ store, actions, go }) {
  const C = window.CAMPUS_DATA;
  const [tab, setTab] = React.useState("catalogue"); // catalogue | collections
  const [adding, setAdding] = React.useState(false);
  const [openColl, setOpenColl] = React.useState(null);
        ...
        <div>
          ...
          <Card className="u-pad" style={{ marginBottom: 16 }}>
            <div className="u-row" style={{ gap: 12 }}>
              <span className="u-icon" style={{ width: 40, height: 40 }}><Icon name={active.icon} size={19} /></span>
...
Object.assign(window, { AddToCollectionModal, LibraryShelf, allLibBooks, allCollections, collectionBookIds });
