...
function LockedGate({ icon, title, sub, go }) {
  return (
...
function Registration({ store, actions, go }) {
  ...
  const initial = COURSES.filter((c) => c.carryover || c.type === "Core").map((c) => c.code);
  const [sel, setSel] = React.useState(store.registration.courses.length ? store.registration.courses : initial);
        ...
        <Card>
          ...
          <div className="u-list" style={{ padding: "0 8px 8px" }}>
            {COURSES.map((c) => {
              ...
              return (
                <button key={c.code} className="u-row" disabled={locked} onClick={() => toggle(c)}
                  style={{ gap: 12, padding: "12px 14px", border: 0, borderTop: "1px solid var(--border)", background: on ? "var(--bg-sunken)" : "transparent", color: "inherit", cursor: locked || c.carryover ? "default" : "pointer", textAlign: "left", width: "100%", opacity: locked && !on ? 0.5 : 1 }}>
...
/* ============ HOSTEL ALLOCATION ============ */
function Hostel({ store, actions, go }) {
  const { HOSTELS, STUDENT, fmt } = window.DATA;
  ...
  if (store.hostel.status === "allocated") {
    return (
      <div className="u-content u-content--narrow">
        ...
        <AllocationSlip store={store} fmt={fmt} />
      </div>
    );
  }
          ...
          <Card className="u-pad">
            <div className="u-row" style={{ justifyContent: "space-between", marginBottom: 14 }}>
              ...
              <Seg value={block?.id || hostel.blocks[0].id}
                onChange={(id) => { setBlock(hostel.blocks.find((b) => b.id === id)); setRoom(null); setBed(null); }}
      ...
      {step === 3 && bed && (
        <div className="u-cols u-cols--aside">
          <Card className="u-pad">
            ...
            {[["Hall", hostel.name], ["Block", "Block " + block.id], ["Room", room.number], ["Bed space", bed.label], ["Occupancy", room.capacity + "-person room"]].map(([k, v]) => (
...
function AllocationSlip({ store, fmt }) {
    ...
    <Card className="u-print-area">
      ...
      <div className="u-pad">
        <div className="u-grid u-grid--2" style={{ gap: 0 }}>
          <div>
            {[["Student", STUDENT.name], ["Matric no.", STUDENT.matric], ["Session", STUDENT.session], ["Gender", STUDENT.gender]].map(([k, v]) => (
              <div key={k} className="u-slip__row"><span className="k">{k}</span><span className="v">{v}</span></div>
            ))}
          </div>
          <div>
            {[["Hall", h.hostelName], ["Block", "Block " + h.block], ["Room", h.room], ["Bed space", h.bedLabel], ["Reference", h.ref]].map(([k, v]) => (
              <div key={k} className="u-slip__row"><span className="k">{k}</span><span className="v">{v}</span></div>
            ))}
          </div>
        </div>
        <div className="u-slip__row" style={{ borderTop: "1px solid var(--border-strong)", marginTop: 6 }}>
          ...
          <span style={{ fontWeight: 700 }} className="u-num">{fmt(h.price)}</span>
        </div>
...
Object.assign(window, { Registration, Hostel });
