...
function PaymentRows({ store, actions, limit, only }) {
  const { PAYMENTS, fmt } = window.ROLE_DATA;
  const pst = (p) => rstate(store, "bursary", "pay", p.id, p.baseStatus);
  let rows = PAYMENTS; if (only) rows = rows.filter((p) => pst(p) === only); if (limit) rows = rows.slice(0, limit);
...
function BursaryVerify({ store, actions }) {
  ...
  return (
    <div className="u-content">
      <PageHead title="Payment Verification" sub="Confirm or flag unverified transactions">
        <Seg value={filter} onChange={setFilter} options={[
          { value: "pending", label: "Pending · " + (counts.pending || 0) },
          { value: "confirmed", label: "Confirmed" }, { value: "flagged", label: "Flagged" },
        ]} />
      </PageHead>
      <Card className="u-pad"><PaymentRows store={store} actions={actions} only={filter} /></Card>
    </div>
  );
}
...
function BursaryDebtors() {
  ...
  return (
    <div className="u-content">
      <PageHead title="Debtors" sub={DEBTORS.length + " students with outstanding payments"}>
...
function HostelAppRows({ store, actions, limit, only }) {
  const { HOSTEL_APPS } = window.ROLE_DATA;
  const hst = (a) => rstate(store, "hostel", "app", a.id, a.baseStatus);
  let rows = HOSTEL_APPS; if (only) rows = rows.filter((a) => hst(a) === only); if (limit) rows = rows.slice(0, limit);
...
function HostelApplications({ store, actions }) {
  ...
  return (
    <div className="u-content">
      <PageHead title="Hostel Applications" sub="Allocate bed spaces to eligible students">
        <Seg value={filter} onChange={setFilter} options={[
          { value: "pending", label: "Pending · " + (counts.pending || 0) },
          { value: "allocated", label: "Allocated" }, { value: "rejected", label: "Declined" },
        ]} />
      </PageHead>
      <Card className="u-pad"><HostelAppRows store={store} actions={actions} only={filter} /></Card>
    </div>
  );
}
...
function IctSessions() {
  const { SESSIONS } = window.ROLE_DATA;
  return (
    <div className="u-content">
      <PageHead title="Sessions & Semesters" sub="Configure academic sessions and registration windows">
        <Btn variant="accent" icon="plus">New session</Btn>
      </PageHead>
      <Card>
        <div style={{ overflowX: "auto" }}>
          <table className="u-table">
            ...
            <tbody>
              {SESSIONS.map((s) => (
                <tr key={s.name}>
                  ...
                  <td><Tag variant={s.state === "Active" ? "success" : undefined} dot={s.state === "Active"}>{s.state}</Tag></td>
                  <td><Tag variant={s.reg === "Open" ? "accent" : undefined}>{s.reg}</Tag></td>
                  <td><Tag variant={s.fees === "Open" ? "accent" : undefined}>{s.fees}</Tag></td>
                  <td className="u-right">{s.state === "Active" ? <Btn variant="secondary" size="sm">Manage</Btn> : <Btn variant="ghost" size="sm">View</Btn>}</td>
                </tr>
              ))}
            </tbody>
          </table>
