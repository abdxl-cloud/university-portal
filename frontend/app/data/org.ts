...
/** A department carrying its owning faculty, as returned by the indexes. */
...
/** Which faculty each dean role maps to. */
export type DeanRole = "dean" | "dean2";
...
// ---- indexes & lookups ----
const _deptIndex: Record<string, DepartmentWithFaculty> = {};
const _facByDept: Record<string, Faculty> = {};
const _facById: Record<string, Faculty> = {};
FACULTIES.forEach((f) => {
  _facById[f.id] = f;
  f.departments.forEach((d) => {
    _deptIndex[d.id] = { ...d, facultyId: f.id, facultyName: f.name, facultyShort: f.short };
    _facByDept[d.id] = f;
  });
});

export const facultyById = (id: string): Faculty | null => _facById[id] || null;
export const deptById = (id: string): DepartmentWithFaculty | null => _deptIndex[id] || null;
export const facultyOfDept = (deptId: string): Faculty | null => _facByDept[deptId] || null;

/** Resolve a faculty from a department NAME or id, OR a faculty name/short (handles legacy string data). */
export const facultyOf = (key: string | null | undefined): Faculty | null => {
  if (!key) return null;
...
export const facultyNameOf = (deptKey: string | null | undefined): string | null => {
  const f = facultyOf(deptKey);
  return f ? f.name : null;
};

export const departmentsOf = (facultyId: string): Department[] =>
  _facById[facultyId] ? _facById[facultyId].departments : [];

export const allDepartments = (): DepartmentWithFaculty[] => Object.values(_deptIndex);

/** Which faculty each dean role maps to. */
export const DEAN_FACULTY: Record<DeanRole, string> = { dean: "computing", dean2: "engineering" };

/** Grouped namespace mirroring the old `window.ORG` shape. */
export const ORG = {
