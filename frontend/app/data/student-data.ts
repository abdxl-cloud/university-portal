...
import type { IconName, Notification, TagVariant, Tone } from "../types";

export type Grade = "A" | "B" | "C" | "D" | "E" | "F";

/** Tag colour for each letter grade. */
export const GRADE_TONE: Record<Grade, TagVariant> = {
  A: "success", B: "accent", C: "accent", D: "warning", E: "warning", F: "danger",
};

/** Grade point per letter grade (5.00 scale). */
export const GRADE_POINT: Record<Grade, number> = { A: 5, B: 4, C: 3, D: 2, E: 1, F: 0 };

/** Where an assignment starts out for the demo student. */
export type AssignmentSeed = "graded" | "submitted" | "open";

export type CourseType = "Core" | "Elective" | "GST" | "Carryover";
...
/** An on-demand charge. `once: true` means payable once per session. */
...
function makeRooms(block: string, count: number, freeArr: number[]): Room[] {
  const rooms: Room[] = [];
...
export const RESULTS: ResultTerm[] = [
  {
...
export const GPA_HISTORY: GpaPoint[] = [
...
export function classFor(code: string): ClassContent {
...
/** Course codes the student has already passed (grade !== F across released results). */
export const passedCourses = (): Set<string> => {
  const s = new Set<string>();
  RESULTS.forEach((r) => r.courses.forEach((c) => { if (c.grade !== "F") s.add(c.code); }));
  return s;
};

export const courseName = (code: string): string => COURSE_TITLES[code] || code;

/** Total obligatory school fees for the session. */
export const totalFees = (): number => FEES.items.reduce((s, i) => s + i.amount, 0);

export const fmt = (n: number): string => "₦" + n.toLocaleString("en-NG");

/** Grouped namespace mirroring the old `window.DATA` shape. */
export const DATA = {
  STUDENT, FEES, CHARGES, PAYMENT_HISTORY, COURSES, COURSE_TITLES, HOSTELS, RESULTS, TIMETABLE,
  ANNOUNCEMENTS, NEWS, GPA_HISTORY, LECTURERS, VENUES, NOTIFICATIONS,
  CLASS_CONTENT, classFor, passedCourses, courseName, fmt,
};
