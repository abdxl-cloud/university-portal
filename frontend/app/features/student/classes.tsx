...
import { Avatar, Btn, Card, Field, Modal, ModalHead, PageHead, Seg, Tag } from "../../components/ui";
import { Ref } from "../../components/detail";
import { COURSES, classFor } from "../../data/student-data";
import { nowStr } from "../../store/store";
import type { ClassAssignment, ClassContent, Course, Material } from "../../data/student-data";
import type { AssignmentSubmission, Store } from "../../store/types";
import type { Actions } from "../../store/store";
import type { TagVariant, Tone } from "../../types";

const FILE_TONE: Record<string, string> = { PDF: "danger", PPTX: "warning", ZIP: "accent", SQL: "accent", DOC: "accent" };

/** Resolve the effective status of an assignment for this student. */
export function asgStatus(a: ClassAssignment, store: Store): AssignmentSubmission {
...
const STATUS_META: Record<AssignmentSubmission["status"], { tone: TagVariant; label: string }> = {
  open: { tone: "warning", label: "To do" },
  submitted: { tone: "accent", label: "Submitted" },
  graded: { tone: "success", label: "Graded" },
};

export function approvedClassCourses(store: Store): Course[] {
...
/** An open assignment, tagged with the course it belongs to. */
export interface PendingAssignment extends ClassAssignment {
  code: string;
  /** Due date as a timestamp, for sorting; Infinity when unparseable. */
  _due: number;
}

/** Open assignments across approved courses, soonest due first. */
export function pendingAssignments(store: Store): PendingAssignment[] {
...
/* ---------------- list ---------------- */
export function Classes({ store, actions, go }: ClassesProps) {
  const [open, setOpen] = React.useState<string | null>(null); // course code
...
type ClassTab = "stream" | "materials" | "assignments";

function ClassDetail({ code, store, actions, onBack }: ClassDetailProps) {
  const course = COURSES.find((c) => c.code === code);
  const content = classFor(code);
  const [tab, setTab] = React.useState<ClassTab>("stream");
  const [submitFor, setSubmitFor] = React.useState<ClassAssignment | null>(null);
  ...
  return (
    <div className="u-content">
      ...
      {submitFor && (
        <SubmitModal a={submitFor} actions={actions} onClose={() => setSubmitFor(null)} />
      )}
    </div>
  );
}

function Stream({ content }: { content: ClassContent }) {
  return (
...
function Materials({ content }: { content: ClassContent }) {
...
interface AssignmentsProps {
  ...
  store: Store;
  onSubmit: (a: ClassAssignment) => void;
}

function Assignments({ content, store, onSubmit }: AssignmentsProps) {
  return (
...
function SubmitModal({ a, actions, onClose }: SubmitModalProps) {
