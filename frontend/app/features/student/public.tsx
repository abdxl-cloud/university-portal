...
import { Btn, Card, Field, IconBtn, Tag } from "../../components/ui";
import { NEWS } from "../../data/student-data";
import { ROLE_IDS, ROLE_LABELS, type SignableRole } from "../../data/roles-data";
import type { IconName } from "../../types";

/** Where the public site can send you. */
export type PublicDest = "home" | "login" | "apply";

export interface PublicNavProps {
  go: (dest: PublicDest) => void;
  dark: boolean;
  setDark: (v: boolean) => void;
}

function PublicNav({ go, dark, setDark }: PublicNavProps) {
...
const PORTAL_TILES: { icon: IconName; t: string; d: string; to: PublicDest }[] = [
...
export function PublicHome({ go, dark, setDark }: PublicHomeProps) {
  return (
    <div className="u-pub">
...
/** Demo staff IDs, one per signable role. */
const STAFF_IDS: Record<SignableRole, string> = {
...
/** Who is signing in: the demo student, or one of the staff roles. */
export type LoginRole = "student" | SignableRole;
...
export function Login({ go, onLogin, dark }: LoginProps) {
  ...
  const lastRoleIsStaff = lastRole !== "student" && ROLE_IDS.includes(lastRole as SignableRole);
  const [role, setRole] = React.useState<"student" | "staff">(lastRole === "student" ? "student" : "staff");
  const [staffRole, setStaffRole] = React.useState<SignableRole>(lastRoleIsStaff ? (lastRole as SignableRole) : "lecturer");
  const presets = {
    student: { id: "FUT/2022/CSC/10428", label: "Matriculation number", sub: "Sign in with your matriculation number to continue." },
    staff: { id: STAFF_IDS[staffRole], label: "Staff ID", sub: "Choose your role and sign in to continue." },
  };
  const [matric, setMatric] = React.useState(presets[role].id);
  const [pw, setPw] = React.useState("demo1234");
  const pickRole = (r: "student" | "staff") => { setRole(r); setMatric(r === "student" ? presets.student.id : STAFF_IDS[staffRole]); };
  const pickStaffRole = (sr: SignableRole) => { setStaffRole(sr); setMatric(STAFF_IDS[sr]); };
  const submit = (e: React.FormEvent) => { e.preventDefault(); onLogin(role === "student" ? "student" : staffRole); };
          ...
          {role === "staff" && (
            <div style={{ marginBottom: 16 }}>
              <Field label="Sign in as">
                <select className="fb-input" value={staffRole} onChange={(e) => pickStaffRole(e.target.value as SignableRole)}>
                  {ROLE_IDS.map((id) => <option key={id} value={id}>{ROLE_LABELS[id]}</option>)}
                </select>
              </Field>
            </div>
          )}
