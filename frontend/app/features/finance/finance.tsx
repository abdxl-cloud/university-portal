...
import { Btn, Card, Modal, ModalHead, PageHead, SkeletonBlock, Stat, Tag, useSkeleton } from "../../components/ui";
import { CHARGES, FEES, PAYMENT_HISTORY, STUDENT, fmt, totalFees } from "../../data/student-data";
import type { Charge } from "../../data/student-data";
import type { Store } from "../../store/types";
import type { Actions } from "../../store/store";
import type { IconName, TagVariant } from "../../types";

const METHOD_LABELS: Record<string, string> = { card: "Debit card", transfer: "Bank transfer", ussd: "USSD" };
...
export function PayModal({ title, sub, amount, onClose, onPaid }: PayModalProps) {
...
/** One row of the unified payment ledger. */
...
function ledger(store: Store): LedgerRow[] {
  ...
  if (store.hostel && store.hostel.status === "allocated") {
    rows.push({ id: "hostel-cur", label: "Hostel Fee: " + store.hostel.hostelName, amount: store.hostel.price ?? 0,
  ...
  (store.payments || []).forEach((p) => rows.push({ ...(p as unknown as LedgerRow), session: FEES.session }));
...
function chargeStatus(charge: Charge, store: Store): ChargeStatus {
...
export function Finance({ store, actions, go }: FinanceProps) {
  const [payFor, setPayFor] = React.useState<Charge | null>(null);
  const loadingHistory = useSkeleton();
  const feesTotal = totalFees();
  ...
  const confirmCharge = (method: string) => {
    if (!payFor) return;
    actions.payCharge({
      chargeId: payFor.id, label: payFor.label, amount: payFor.amount,
      method: METHOD_LABELS[method] || "Card", category: payFor.group,
    });
    setPayFor(null);
  };
                  ...
                  return (
                    <Card key={c.id} className="u-pad u-stack" style={{ gap: 10 }}>
                      <div className="u-row" style={{ justifyContent: "space-between" }}>
                        ...
                        {st.paid ? <Tag variant="success" dot>Paid</Tag> : c.once ? <Tag>One-time</Tag> : (st.count ?? 0) > 0 ? <Tag>{"×" + st.count} paid</Tag> : null}
                      </div>
                      <div>
            ...
            <tbody>
              {loadingHistory ? (
                ...
                <tr key={r.id}>
                  ...
                  <td><Tag variant={(r.category === "Obligatory" ? "accent" : r.category === "Penalties" ? "warning" : undefined) as TagVariant | undefined}>{r.category}</Tag></td>
