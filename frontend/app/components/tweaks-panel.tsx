...
/** An option is either a bare value or an explicit value/label pair. */
...
export function useTweaks<T extends object>(defaults: T): [T, SetTweak<T>] {
  ...
  // JSON block.
  const setTweak = React.useCallback((keyOrEdits: unknown, val?: unknown) => {
...
export interface TweakRadioProps<T extends string | number | boolean> {
  label?: React.ReactNode;
  value: T;
...
export function TweakRadio<T extends string | number | boolean>({ label, value, options, onChange }: TweakRadioProps<T>) {
  ...
  const isPair = (o: TweakOption<T>): o is { value: T; label: string } =>
  ...
  const labelLen = (o: TweakOption<T>) => String(isPair(o) ? o.label : o).length;
  ...
  if (!fitsAsSegments) {
    ...
    const resolve = (s: string): T => {
      const m = options.find((o) => String(isPair(o) ? o.value : o) === s);
      return m === undefined ? (s as T) : isPair(m) ? m.value : m;
    };
    return <TweakSelect label={label} value={value} options={options}
                        onChange={(s) => onChange(resolve(String(s)))} />;
  }
  const opts = options.map((o) => (isPair(o) ? o : { value: o, label: String(o) }));
...
export interface TweakSelectProps<T extends string | number | boolean> {
  label?: React.ReactNode;
  value: T;
  options: TweakOption<T>[];
...
export function TweakSelect<T extends string | number | boolean>({ label, value, options, onChange }: TweakSelectProps<T>) {
  const isPair = (o: TweakOption<T>): o is { value: T; label: string } =>
    typeof o === "object" && o !== null && "value" in o;
  return (
    <TweakRow label={label}>
      <select className="twk-field" value={String(value)} onChange={(e) => onChange(e.target.value)}>
        {options.map((o) => {
          const v = isPair(o) ? o.value : o;
          const l = isPair(o) ? o.label : o;
...
/** A colour tweak: one hex, or a 1–5 hex palette stored as an array. */
